...
import (
	...
	iappfw "agnione/v2/src/appfm/iappfw"
...
type AgniApp struct {
	...
	appready_status *apptypes.AppReady_Status
	...
	log_entry      chan logger.LogMessage
	console_entry  chan string
...
func (app *AgniApp) Initialize(pCTX_Current *context.Context, pOS_PID *int, pBase_Path *string, pApp_Config *string, pLog_Path *string,
	...
	app.Write2Console("Initializing the Logger with base path ")
	...
	if _err != nil {
		app.Write2Console("Failed to create the instance of Logger.\n " + _err.Error())
		app.logger = nil
	} else {
		if !app.logger.Start() {
	...
	app.Write2Console("Application " + app.name + " - " + app.version + " loaded")
...
func (app *AgniApp) DeInitialize() {
	...
	close(app.log_message)
	app.log_message = nil

	close(app.log_entry)
	app.log_entry = nil
	if app.SSEMonitor != nil {
		app.SSEMonitor.DeInitialize()
	}
...
func (app *AgniApp) Start() {
	...
	/// do the first memory usage read
	defer func() {
		...
		go func() {
			...
			go app.read_memory_usage(bDone)
			<-bDone
		}()
	}()
...
func (app *AgniApp) Stop_HTTPMonitor() {
	...
	if app.HTTPMonitor != nil {
		if app.HTTPMonitor.IsStarted() {
			if app.SSEMonitor.IsStarted() {
				app.Write2Log("Stopping SSE monitoring ........", apptypes.LOG_INFO)
				app.SSEMonitor.Stop()
				app.SSEMonitor.DeInitialize()
				app.SSEMonitor = nil
				app.appinfo.SSEMonitor_Started = false
				app.Write2Log("Stopping SSE monitoring ....... DONE", apptypes.LOG_INFO)
			}
		}
